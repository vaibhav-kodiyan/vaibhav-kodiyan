...
> 2. A **classic** GitHub personal access token exists — Settings → Developer settings → Personal access tokens → **Tokens (classic)** (not "Fine-grained tokens", which GitHub shows first by default; fine-grained tokens aren't accepted for GraphQL auth and this action needs GraphQL) → Generate new token (classic), no scopes needed for public data
